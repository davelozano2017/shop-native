...
function add_to_cart(product_id,status) {
    ...
    var button = (status === 'Delete') ? $('#add_to_cart'+product_id).html('<i class="icon icon-shopping-cart"></i> Add to cart') : $('#add_to_cart'+product_id).html('<i class="icon icon-check"></i> Added');
...
function show_cart_counts() {
    $.ajax({type:'POST',data:{action:'Show Cart Counts'},url:'shop-native/'+url,success:function(data){$('#show_cart_counts').html(data);}});
}
